  ...
  return (
    <div className={styles.playerResults}>
      <div className={styles.summaryEventsWrapper}>
        ...
        {/* {myProfile && ( */}
        <div className={`${styles.card} ${styles.recentSessionReports}`}>
          <div className={styles.recentSessionReportsTitle}>Recent Session Reports</div>
          <div>
            {currentProfile.recent_events.map(event => (
              <>
                <div>
                  <div
                    className={styles.recentSessionReportsWrapperInfo}
                    onClick={() => history.push(`/event/${event.id}/${currentProfile.id}`)}
                  >
                    ...
                    <div>
                      ...
                      <div>{new Date(event.date).getMonth() + 1}</div>
                      <div>{new Date(event.date).getDate()}</div>
                    </div>
                    <div> {event.data_rows_count} Pitches Seen</div>
                    <div> {event.event_name} </div>
                    <div>
                      {event.recent_avatars.map(player => (
                        <div key={player.id} onClick={() => history.push(`/profile/${player.id}`)}></div>
                      ))}
                    </div>
                  </div>
                </div>
